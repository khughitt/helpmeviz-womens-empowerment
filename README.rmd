...
values = world_bank  %>% select(-Country.Name, -Country.Code, -Indicator.Code)
world_bank$mean_stunting = apply(values, 1, function(x) { mean(x, na.rm=T) })

# Most recent data point for each Country for which data is available during
# this time period
world_bank$most_recent_stunting = as.numeric(apply(values, 1, function(x) {
                                                 tail(x[!is.na(x)], 1) }))
...
world_bank = world_bank %>% filter(!is.nan(mean_stunting))
