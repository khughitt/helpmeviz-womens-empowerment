...
df = tbl_df(read.csv('input/Gender_Inequality_Index.csv', col.names=gii_cols,
                     na.strings='..'))

# Drop unneeded fields
df = df %>% select(-hdi_rank, -seats_parlim_female_note,
...
# Add to main dataset
df = merge(df, ifpri_dat, by='country')
```
...
world_bank = tbl_df(read.csv('input/World Bank Stunting and Wasting Data, by gender.csv'))

# Grab data for the last 15 years
world_bank = world_bank %>% select(Country.Name, Country.Code, Indicator.Code,
...
averages = world_bank %>% select(-Country.Name, -Country.Code, -Indicator.Code)
world_bank$mean = apply(averages, 1, function(x) { mean(x, na.rm=T) })

# Most recent data point for each Country for which data is available during
# this time period
world_bank$most_recent = as.numeric(apply(world_bank_stunting, 1, function(x) {
                                        tail(x[!is.na(x)], 1)
                                    }))
...
world_bank = world_bank %>% filter(!is.nan(mean))

# Male vs. Female Stunting (height)
male_stunting = world_bank %>% filter(Indicator.Code == 'SH.STA.STNT.MA.ZS')
female_stunting = world_bank %>% filter(Indicator.Code == 'SH.STA.STNT.FE.ZS')

# Male vs. Female Malnutrition (weight)
male_malnutrition = world_bank %>% filter(Indicator.Code == 'SH.STA.MALN.MA.ZS')
female_malnutrition = world_bank %>% filter(Indicator.Code == 'SH.STA.MALN.FE.ZS')

world_bank_dat = data.frame(
...
df = merge(df, world_bank_dat, by='country')
```
